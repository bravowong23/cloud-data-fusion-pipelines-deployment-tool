...
read -p "Are you sure want to disable all pipelines schedules in your CDAP environment (y/n)?" CONT
if [ "$CONT" = "y" ]; then
  ...
  echo `curl -s --location --request POST ${cdap_endpoint}'/v3/namespaces/default/apps/'$(_jq '.name')'/schedules/dataPipelineSchedule/disable' --header 'Authorization: Bearer '$access_token` > /dev/null
  echo $(_jq '.name')' schedule disabled.'
  done
